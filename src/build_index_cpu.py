...
def estimate_database_parameters(file_path, window_size, stride):
    """
    Dynamically calculates N (total vectors), nlist (clusters), 
    and T (training size) based on file size and compression status.
    """
    file_bytes = os.path.getsize(file_path)
    
    # Check if the file is compressed to adjust the base pair estimation
    if file_path.endswith('.gz'):
        estimated_bases = file_bytes * 3.5
    else:
        estimated_bases = file_bytes
    ...
    target_t = min(256 * nlist, 5_000_000) 
...
def build_pipeline(fasta_path, db_path, index_path, window_size=100, stride=50, batch_size=100000):
    ...
    model = MockRoPEModel(embedding_dim=512)
    embedder = SequenceEmbedder(model, device=device)
    indexer = FaissIndexer(embedding_dim=512, nlist=nlist, m=8)
    ...
    full_training_matrix = np.empty((len(training_strings), 512), dtype=np.float32)
    ...
    print(f"\n[3/5] Training FAISS (This relies heavily on CPU RAM)...")
